...
\subsubsection{Latecomer pension rate ($LPR$)}

The latecomer's pension is paid only to the last generation of the pension system. This component ensures that the last generation of the pension system also receives a pension and is intended to create an additional incentive for joining the pension system. The idea is that the latecomer pension is so large that every user wants to be part of the last generation. 
The latecomer pension rate $LPR(p, units)$ defines the conversion rate from DPT to reserves. 
