...
The simulation shows that with the constant and fair behavior and no influence of inflation and deflation there are no winners and no losers, everyone just gets their deposits back without any influence of inflation or deflation in economics.
...
User & Periods & Contributions & Result \\ [0.5ex] 
\hline
1..100 & 1..480 & $\sum_{j=0}^{39} 12 \cdot CCV \cdot 1.05^j = 1450_{Units}$ & $3831_{Units}$ \\ 
100..200 & 480..960 & $\sum_{j=40}^{79} 12 \cdot CCV \cdot 1.05^{j} = 10165_{Units}$ & $7783_{Units}$ \\ [1ex] 
 \hline
 \end{tabular}%
}
\end{table}
...
User & Periods & Contributions & Result \\ [0.5ex] 
\hline
1..100 & 1..480 & $sum(CCV \cdot 1.05^{n}) = 217_{Units}$ & $150_{Units}$ \\ 
100..200 & 480..960 & $sum(CCV \cdot 1.05^{40}) = 31_{Units}$ & $98_{Units}$ \\ [1ex] 
 \hline
 \end{tabular}%
}
\end{table}
